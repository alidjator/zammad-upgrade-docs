...
| 6.0 | 3.1.3 | ≥7.8, <9 | Pengumuman resmi: MySQL akan di-drop mulai 7.0 | **Redis jadi hard dependency.** ⚠️ Reverse-proxy WAJIB dikonfigurasi ulang untuk WebSocket (`/cable`) sebelum hop ini |
| — migrasi DB — | — | — | ⚠️ **MariaDB → PostgreSQL WAJIB selesai di sini**, sebelum menyentuh 6.0→7.0 (tool migrasi `rake zammad:db:pgloader` baru ada mulai Zammad 5.3) | Lihat panduan resmi: migrate-to-postgresql.html |
...
Ditandai langsung dengan ⚠️ di tabel di atas (baris 5.0, baris "migrasi DB", dan baris
6.0) — bukan diulang di sini supaya tidak ada dua sumber kebenaran. Ringkas: ES harus
≥7.8 sebelum hop 5.0, migrasi database wajib selesai sebelum hop 7.0, dan reverse
proxy nginx perlu disesuaikan sebelum hop 6.0.
