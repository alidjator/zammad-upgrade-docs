...
- [ ] Catat ukuran data terkini (jumlah tiket, artikel, user) — **bandingkan dengan
      ...
      jika jauh lebih besar, **skalakan ulang estimasi waktu** di
...
**Gate keras:** jika salah satu item di atas gagal/belum siap, **JANGAN mulai
...
- **Image `zammad-app`/`websocket`/`scheduler` WAJIB pakai `image:` yang SAMA** di
  `docker-compose.yml` — jika tidak, build jadi 3x lebih lama (terbukti terjadi di
...
- Pantau disk selama reindex — ES bisa masuk mode `read_only_allow_delete` jika
...
- Migrasi database Zammad historis kadang punya **bug urutan** (migrasi lama butuh
  tabel yang baru dibuat migrasi jauh lebih baru — lihat Insiden 6 hop 6.0→7.0). Jika
...
- [ ] **Batas waktu**: jika downtime keras (build+migrate, biasanya di bawah 1 jam
      ...
      coba tanpa batas.
- [ ] **Batas percobaan**: jika satu masalah yang sama gagal diperbaiki setelah N
...
- [ ] **Satu webhook (jika dipakai) terbukti terkirim**
...
- [ ] Kolom database yang diketahui berganti nama antar hop (lihat CHANGELOG tiap
      hop) — cek satu integrasi/laporan yang bergantung pada kolom tersebut, jika ada
...
- [ ] **Siapa** yang perlu diberi tahu: agent/tim support internal (wajib), pelanggan
      (jika maintenance window terlihat/mengganggu akses mereka)
