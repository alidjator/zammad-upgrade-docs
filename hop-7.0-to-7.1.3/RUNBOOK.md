...
- [ ] Cek disk space (`df -h /`, `docker system df`) — bersihkan dulu kalau di bawah
...
```
Kalau ada migrasi baru dengan pola bug urutan seperti `recent_closes` di hop
...
```
Kalau 500 atau file CSS tidak ada, jalankan precompile manual + restart (lihat
...
```
Kalau ada index `zammad_production_*` yang seharusnya sudah tidak ada, hapus dulu
...
```
**Pantau disk selama proses ini berjalan** (`df -h /` sesekali) — kalau mendekati 95%,
...
- [ ] Search tiket berfungsi (query langsung ke ES kalau mau verifikasi lebih pasti:
